...
@login_required
def pai_messages(request, recipients=None, template_name='django_messages/pai_messages.html'):
    name = request.GET.get("name", None)
    about = request.GET.get('about', None)
    if recipients is not None:
        recipients = [u.public_key for u in User.objects.filter(
            **{'%s__in' % get_username_field(): [r.strip() for r in recipients.split('++')]})]

    thread = {"recipients": recipients,
              "about": about,
              "name": name}

    application = {"namespace": settings.CATENA_PAIPASS_NAMESPACE,
